...
Update `CMakeLists.txt` if you used a different OpenCV version — change the two lines that reference `opencv-4.7.0`:

```cmake
set(OpenCV_DIR "$ENV{HOME}/Software/opencv/opencv-4.10.0/build")
...
link_directories("$ENV{HOME}/Software/opencv/opencv-4.10.0/build/lib" ...)
```
