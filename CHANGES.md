...
## 9. Parameter Sweep (`scripts/sweep.py`)

A standalone CLI utility for systematic hyperparameter comparison.

Given a ROS 2 bag path, `sweep.py` runs `gs_mapping` once for each entry in its
`SWEEP` list, saving the resulting point cloud, renders, and runtime statistics to a
timestamped sub-directory. A top-level `summary.txt` lists wall times and exit codes
for all runs at a glance.

Key design details:

- Patches `config/odin1.yaml` on the fly with per-run overrides using a temporary YAML
  file; the base config is never modified.
- Detects node readiness by watching for the `"Publishing:"` log line before starting
  bag playback, avoiding dropped frames at startup.
- Strips ANSI colour codes before writing `stats.txt` so the output is clean in editors.
- `RESULTS_ROOT` defaults to `/home/jetson/results` on this branch; override via the
  `RESULTS_ROOT` environment variable.
- `INIT_WAIT_S` is set to 20 s to accommodate TRT engine loading time on Jetson.

The default `SWEEP` preset covers five combinations ranging from the real-time Orin
settings (`max_iters=15`, depth off) to a high-quality offline configuration
(`max_iters=100`, depth on, tight pruning). Run 5 is not suitable for live use on
Orin NX 8 GB.
...
| `scripts/sweep.py` | Hyperparameter sweep — runs `gs_mapping` with multiple config overrides over a bag |
