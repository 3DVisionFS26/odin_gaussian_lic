...
// ROS2: tf includes removed; pose conversions are done inline below
...
{
    /// image – unpack directly from the pre-built sensor_msgs::msg::Image (BGR8)
    cv::Mat image_bgr(cur_frame.image_msg->height, cur_frame.image_msg->width,
                      CV_8UC3,
                      const_cast<uint8_t*>(cur_frame.image_msg->data.data()),
                      cur_frame.image_msg->step);
    image_bgr = image_bgr.clone();   // own the data before converting
    cv::Mat image_rgb;
    cv::cvtColor(image_bgr, image_rgb, cv::COLOR_BGR2RGB);  // 0-255
    image_rgb.convertTo(image_rgb, CV_32FC3, 1.0f / 255.0f);  // 0-1

    /// depth – unpack directly from the pre-built sensor_msgs::msg::Image (32FC1)
    cv::Mat depth_map(cur_frame.depth_msg->height, cur_frame.depth_msg->width,
                      CV_32FC1,
                      const_cast<uint8_t*>(cur_frame.depth_msg->data.data()),
                      cur_frame.depth_msg->step);
    depth_map = depth_map.clone();   // own the data

    /// pose – inline conversion from geometry_msgs::msg::PoseStamped
    const auto& ori = cur_frame.pose_msg->pose.orientation;
    const auto& pos = cur_frame.pose_msg->pose.position;
    Eigen::Quaterniond q_wc(ori.w, ori.x, ori.y, ori.z);
    q_wc.normalize();
    Eigen::Vector3d t_wc(pos.x, pos.y, pos.z);
    ...
    if ((all_frame_num_ + 1) % select_every_k_frame_ == 0)
    {
        ...
        {
            auto& stamp = cur_frame.image_msg->header.stamp;
            std::stringstream ss;
            ss << stamp.sec << "_" << std::setw(9) << std::setfill('0') << stamp.nanosec;
            cam->image_name_ = ss.str() + ".jpg";
        }
...
GaussianModel::GaussianModel(const Params& prm)
{
    ...
    prune_opacity_thresh_    = prm.prune_opacity_thresh;
    prune_every_n_keyframes_ = prm.prune_every_n_keyframes;
...
void GaussianModel::pruneGaussians(const torch::Tensor& mask)
{
    // mask: bool [N], true = keep
    auto keep_idx = mask.nonzero().squeeze(1);
    ...
    for (int group_idx = 0; group_idx < 6; ++group_idx)
    {
        ...
        param = param.index_select(0, keep_idx).requires_grad_();
        group.params()[0] = param;
        auto new_param_impl = param.unsafeGetTensorImpl();
        ...
        {
            auto stored_state = state_it->second;
            stored_state.exp_avg    = stored_state.exp_avg.index_select(0, keep_idx);
            stored_state.exp_avg_sq = stored_state.exp_avg_sq.index_select(0, keep_idx);
            optimizer_state.erase(state_it);
            optimizer_state[new_param_impl] = stored_state;
        }
    }

    this->xyz_           = param_groups[0].params()[0];
    this->features_dc_   = param_groups[1].params()[0];
    this->features_rest_ = param_groups[2].params()[0];
    this->opacity_       = param_groups[3].params()[0];
    this->scaling_       = param_groups[4].params()[0];
    this->rotation_      = param_groups[5].params()[0];
...
double optimize(const std::shared_ptr<Dataset>& dataset, std::shared_ptr<GaussianModel>& pc, int max_iters)
{
                           ...
                           const std::string& lpips_path)
{
    ...
    // Save the Gaussian map immediately so it's on disk even if rendering crashes
    pc->saveMap(result_path);
