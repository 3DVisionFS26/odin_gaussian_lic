...
public:
    Dataset(const Params& prm)
      : fx_(prm.fx), fy_(prm.fy), cx_(prm.cx), cy_(prm.cy),
        ...
        depth_completer_(prm.depth_completion ? prm.engine_path : "", prm.width, prm.height) {}
...
public:
    ...
    void pruneGaussians(const torch::Tensor& mask);
...
public:
    ...
    double prune_opacity_thresh_;
    int prune_every_n_keyframes_;
...
double optimize(const std::shared_ptr<Dataset>& dataset, std::shared_ptr<GaussianModel>& pc, int max_iters = 100);
