...
{
    if (enginePath.empty()) return;  // disabled — skip engine load and CUDA stream
...
cv::Mat DepthCompleter::complete(const cv::Mat& rgbImage, const cv::Mat& depthImage) 
{
    ...
    for (size_t i = 0; i < mTensorNames.size(); i++)
        mContext->setTensorAddress(mTensorNames[i].c_str(), mDeviceBuffers[i]);
...
void DepthCompleter::allocateBuffers()
{
    ...
    {
        mTensorNames[i] = mEngine->getIOTensorName(i);
        nvinfer1::Dims dims = mEngine->getTensorShape(mTensorNames[i].c_str());
...
void DepthCompleter::prepareInputs(const cv::Mat& rgbImage, const cv::Mat& depthImage) 
{
    ...
    // Copy to device (async on inference stream)
    for (size_t i = 0; i < mDeviceBuffers.size() - 1; i++)
    {
        if (cudaMemcpyAsync(mDeviceBuffers[i], mHostBuffers[i].data(),
...
{
    if (cudaMemcpyAsync(mHostBuffers.back().data(), mDeviceBuffers.back(),
