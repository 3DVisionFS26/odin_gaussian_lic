...
<div id="main">
  ...
  <div id="sidebar">
    ...
    <div class="card">
      ...
      <span class="lbl">LiDAR points</span>
      <span class="val" id="lidar-pts">—</span>
    </div>
    <div class="card">
...
// ── Accumulated LiDAR point cloud ─────────────────────────────────────────
let lidarMesh    = null;
let lidarVersion = -1;

function loadLidar(serverVersion) {
  if (serverVersion <= lidarVersion) return;
  fetch('/lidar.bin')
    ...
    .then(buf => {
      ...
      const rgbSrc = new Uint8Array(buf, 4 + n * 12, n * 3);
      ...
      for (let i = 0; i < n; i++) {
        ...
        col[i*3]   = rgbSrc[i*3]   / 255;
        col[i*3+1] = rgbSrc[i*3+1] / 255;
        col[i*3+2] = rgbSrc[i*3+2] / 255;
      }
      ...
      lidarMesh    = new THREE.Points(geo, mat);
      lidarVersion = serverVersion;
      scene.add(lidarMesh);
    })
    .catch(() => {});
}
...
function fetchState() {
  fetch('/state.json')
    ...
    .then(d => {
      ...
      loadLidar(d.stats.lidar_version || 0);

      document.getElementById('kf').textContent = d.stats.odom_count || '—';
      const lp = document.getElementById('lidar-pts');
      if (lp) lp.textContent = d.stats.lidar_total > 0
        ? (d.stats.lidar_total / 1000).toFixed(0) + 'k' : '—';
...
# Max points to keep in the accumulated lidar buffer
_MAX_LIDAR_PTS = 500_000
# Points to add per incoming scan (downsample before accumulating)
_SCAN_ADD_PTS  = 3_000
...
class WebMonitorNode(Node):
    def __init__(self):
        ...
        self._lidar_xyz: np.ndarray = np.empty((0, 3), dtype=np.float32)
        self._lidar_rgb: np.ndarray = np.empty((0, 3), dtype=np.uint8)
        self._lidar_bin: bytes = b""
        self._lidar_version: int = 0
        ...
        self.create_subscription(PointCloud2, "/odin1/cloud_slam",  self._lidar_cb,  be_qos)
    ...
    def _lidar_cb(self, msg: PointCloud2) -> None:
        ...
        try:
            ...
            ox = fo.get('x', 0); oy = fo.get('y', 4); oz = fo.get('z', 8)
            orgb = fo.get('rgb', fo.get('rgba', 12))
            dt = np.dtype({
                'names':   ['x', 'y', 'z', 'rgb'],
                'formats':  [np.float32, np.float32, np.float32, np.uint32],
                'offsets':  [ox, oy, oz, orgb],
                'itemsize': stride,
            })
            pts     = np.frombuffer(data, dtype=dt)
            xyz     = np.column_stack([pts['x'], pts['y'], pts['z']]).astype(np.float32)
            rgb_u32 = pts['rgb']
            valid   = np.isfinite(xyz).all(axis=1)
            xyz     = xyz[valid];  rgb_u32 = rgb_u32[valid]
            if len(xyz) == 0:
                return

            # Downsample this scan before accumulating
            step  = max(1, len(xyz) // _SCAN_ADD_PTS)
            xyz_s = xyz[::step]
            r_s   = ((rgb_u32[::step] >> 16) & 0xFF).astype(np.uint8)
            g_s   = ((rgb_u32[::step] >>  8) & 0xFF).astype(np.uint8)
            b_s   = ( rgb_u32[::step]        & 0xFF).astype(np.uint8)
            rgb_s = np.column_stack([r_s, g_s, b_s])

            with self._lock:
                self._lidar_xyz = np.concatenate([self._lidar_xyz, xyz_s])
                self._lidar_rgb = np.concatenate([self._lidar_rgb, rgb_s])

                # Cap total size — randomly subsample when over limit
                n_total = len(self._lidar_xyz)
                if n_total > _MAX_LIDAR_PTS:
                    keep = np.random.choice(n_total, _MAX_LIDAR_PTS // 2, replace=False)
                    self._lidar_xyz = self._lidar_xyz[keep]
                    self._lidar_rgb = self._lidar_rgb[keep]
                    n_total = len(self._lidar_xyz)

                # Build downsampled binary for browser (≤ 30k points)
                step_d  = max(1, n_total // 30000)
                xyz_d   = self._lidar_xyz[::step_d]
                rgb_d   = self._lidar_rgb[::step_d]
                n_d     = len(xyz_d)
                self._lidar_bin = struct.pack("<I", n_d) + xyz_d.tobytes() + rgb_d.tobytes()
                self._lidar_version += 1

        except Exception as exc:
            self.get_logger().warn(f"lidar parse: {exc}", throttle_duration_sec=5.0)
    ...
    def _reset_session_state(self):
        ...
        self._odom_count          = 0
        self._lidar_xyz           = np.empty((0, 3), dtype=np.float32)
        self._lidar_rgb           = np.empty((0, 3), dtype=np.uint8)
        self._lidar_bin           = b""
        self._lidar_version       = 0
        self._latest_jpeg         = b""
        self._input_jpeg          = b""
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if p == "/":
                    ...
                    with node._lock: data = node._lidar_bin
                    self._send_or_204(data, "application/octet-stream")
                elif p == "/state.json":
                    with node._lock:
                        traj    = node._trajectory[-2000:]
                        stats   = {
                            "odom_count":   node._odom_count,
                            "lidar_total":  len(node._lidar_xyz),
                            "lidar_version": node._lidar_version,
                        }
